...
# Compile for Mac
CGO_ENABLED=1 gox -ldflags="-w -X main.version $VERSION -X main.buildDate $BUILD_DATE" -osarch="darwin/amd64" -output="lantern_{{.OS}}_{{.Arch}}" github.com/getlantern/flashlight
# Compile for Windows (use -H=windowsgui ldflag to make this a Windows instead of a console app)
CGO_ENABLED=1 gox -ldflags="-w -X main.version $VERSION -X main.buildDate $BUILD_DATE -H=windowsgui" -osarch="windows/386" -output="lantern_{{.OS}}_{{.Arch}}" github.com/getlantern/flashlight