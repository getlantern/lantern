...
public class SystemExtension: NSObject, OSSystemExtensionRequestDelegate {
    ...
    public func activation() throws -> OSSystemExtensionRequest.Result? {
        print("Activating system extension...")
        let request = OSSystemExtensionRequest.activationRequest(forExtensionWithIdentifier: "org.getlantern.lantern.system", queue: .main)
        request.delegate = self
        OSSystemExtensionManager.shared.submitRequest(request)
        print("Submitted activation request...")
        semaphore.wait()
        if let error {
            throw error
        }
