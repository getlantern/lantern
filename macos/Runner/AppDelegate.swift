...
class AppDelegate: FlutterAppDelegate {
  ...
  private func setupRadiance() {
    ...
    appLogger.info(
      "Setting up Radiance in \(opts.env), logging to \(opts.logDir), dataDir: \(opts.dataDir), telemetryConsent: \(opts.telemetryConsent), locale: \(opts.locale)"
    )
    var error: NSError?
    ...
    if let error {
      ...
      appLogger.error("Radiance setup did not complete")
    } else {
