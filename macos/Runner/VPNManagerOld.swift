...
class VPNManagerOld: ObservableObject {
  ...
  func activateVPN(bundleID: String) {
  ...
  func loadExistingVPNManager(targetLocalizedDescription: String? = nil) async -> Result<
    NETunnelProviderManager, VPNLoadError
  > {
    ...
    do {
      ...
      if let targetDesc = targetLocalizedDescription {
        ...
        if let specificManager = managers.first(where: { $0.localizedDescription == targetDesc }) {
          ...
          logger.warning(
            "Specific VPN configuration '\(targetDesc)' not found among loaded managers.")
        ...
        if managers.count == 1 {
          ...
          logger.warning(
            "Multiple VPN configurations found, but no specific target. Returning the first one.")
