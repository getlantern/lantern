...
# BRAND-ONLY native scrub. Go embeds getlantern import paths in
# .gopclntab/.rodata that source edits cannot remove, so the brand is scrubbed
# here (same mapping as the source de-brand: lantern->backend, getlantern->
# foundation, length-preserving).
#
# vpn/tun/VpnService/etc. are deliberately NOT scrubbed: (1) they are ALLOWED in
# the stealth-vpn leakage policy (only lantern_identity + social_urls are
# forbidden); (2) scrubbing them CORRUPTS gomobile JNI class-name strings the Go
# runtime looks up by FindClass -- e.g. "foundation/engine/libbox/AndroidVPNType"
# became "AndroidNETType", a ClassNotFoundException that crashed Libbox.init() on
# startup. For stealth-novpn the VPN/tunnel surfaces are removed at the SOURCE
# (Go build tags exclude the code; the manifest declares no VpnService, verified
# by the manifest-diff tests) -- never by token scanning or native string
# scrubbing. The leakage gate's stealth-novpn mode scans brand/social tokens only.
...
def is_native_library(filename: str) -> bool:
    # Only the Go native library (libgojni.so) is byte-scrubbed: Go embeds import
    # paths in .gopclntab/.rodata that source edits and garble cannot fully
    # remove. The Dart snapshot (libapp.so) is NOT scrubbed -- doing so silently
    # rewrites Dart string-literal contents (i18n keys, asset paths) and desyncs
    # them from the .po/asset files. libapp.so is de-branded at the source level
    # (run_flutter_build.py + --obfuscate) instead.
    normalized = filename.replace("\\", "/")
    return normalized.endswith("/libgojni.so") or normalized == "libgojni.so"
