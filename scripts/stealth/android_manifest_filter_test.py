...
- novpn additionally removes VpnService / QuickTile and injects the neutral
  local-connection foreground service (no "Vpn" token in its name).

Expected component names are read from the android_manifest_filter module
constants (STEALTH_*) rather than hard-coded, so a rename in the source does not
silently leave these tests asserting a stale name.
"""
...
class VpnModeTest(unittest.TestCase):
    ...
    def test_launcher_activity_present(self) -> None:
        activity_names = _names(self.app.findall("activity"))
        self.assertIn(f.STEALTH_ACTIVITY_NAME, activity_names)

    def test_launcher_filter_kept(self) -> None:
        for activity in self.app.findall("activity"):
            if activity.attrib.get(f"{ANDROID}name") == f.STEALTH_ACTIVITY_NAME:
                ...
                return
        self.fail(f"{f.STEALTH_ACTIVITY_NAME} not found")
    ...
    def test_application_rewritten(self) -> None:
        self.assertEqual(
            self.app.attrib.get(f"{ANDROID}name"), f.STEALTH_APPLICATION_NAME
        )

    def test_vpn_service_rewritten(self) -> None:
        service_names = _names(self.app.findall("service"))
        self.assertIn(f.STEALTH_VPN_SERVICE_NAME, service_names)
    ...
    def test_tile_service_rewritten(self) -> None:
        service_names = _names(self.app.findall("service"))
        self.assertIn(f.STEALTH_TILE_SERVICE_NAME, service_names)
...
class NoVpnModeTest(unittest.TestCase):
    ...
    def test_quick_tile_removed(self) -> None:
        service_names = _names(self.app.findall("service"))
        self.assertNotIn(f.STEALTH_TILE_SERVICE_NAME, service_names)

    def test_local_connection_service_injected(self) -> None:
        service_names = _names(self.app.findall("service"))
        self.assertIn(f.STEALTH_NOVPN_SERVICE_NAME, service_names)

    def test_injected_service_name_has_no_vpn_token(self) -> None:
        # The no-VPN foreground service must not advertise VPN heritage in its
        # (logcat- and manifest-visible) component name.
        self.assertNotIn("Vpn", f.STEALTH_NOVPN_SERVICE_NAME)
        for name in _names(self.app.findall("service")):
            self.assertNotIn("Vpn", name,
                msg=f"service {name!r} carries a 'Vpn' token in novpn mode")
