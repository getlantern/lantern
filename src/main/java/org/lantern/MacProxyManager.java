...
public class MacProxyManager {
    private static final Logger log = 
        LoggerFactory.getLogger(MacProxyManager.class);
    ...
    public void changeNetworkSettings() throws IOException {
        ...
        customProxyPACDir = createCustomProfileDir(sessionId);
        if (customProxyPACDir.exists()) {
            try {
                FileUtils.deleteDirectory(customProxyPACDir);
            } catch (IOException e) {
                log.warn("Error deleting directory?", e);
            }
        }
    ...
    public static File createCustomProfileDir(String sessionId) {
      ...
      customProfileDir = customProfileDir(sessionId);
        if (customProfileDir.exists()) {
            try {
                FileUtils.deleteDirectory(customProfileDir);
            } catch (IOException e) {
                log.warn("Error deleting directory?", e);
            }
        }
    ...
    private String findNetworkSetupBin() {
        ...
        if (defaultLocation.getParentFile().exists()) {
            ...
            if (guess != null) {
                ...
                log.error("Couldn't find 'networksetup' in expected location; we're taking " +
    ...
    private String chooseSuitableNetworkSetup(String osVersion, String osArch, String... files) {
        ...
        if (candidates.isEmpty()) {
            log.trace("No networksetup candidates found");
            return null;
        }
        if (candidates.size() == 1) {
            log.trace("One networksetup candidate found");
            return candidates.iterator().next();
        }
        log.trace("Multiple networksetup candidates found: " + candidates);
        ...
        if (versionParts.length < 2) {
            log.trace("OS version seems to be invalid: " + osVersion);
            return null;
        }
        if (!"10".equals(versionParts[0])) {
            log.trace("OS version doesn't seem to be 10.*: " + osVersion);
            return null;
        }
        CodeName codeName;
        try {
            ...
            if (candidates.contains(possibleCandidate)) {
                log.trace("This seems to be " + codeName + ", so we'll use " + possibleCandidate);
                return possibleCandidate;
            }
            log.trace("This seems to be " + codeName + ", but there's no " + possibleCandidate);
        } catch (IllegalArgumentException e) {
            log.trace("Couldn't find code name for OS version " + osVersion);
            return null;
        }
    ...
    public String runNetworkSetup(final String... args) {
    ...
    public Collection<String> getNetworkServices() {
        final String services = runNetworkSetup("-listallnetworkservices");
        log.info("Services: {}", services);
        final String[] strs = services.split("\n");
