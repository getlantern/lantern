...
public class S3Config {
    ...
    public Collection<FallbackProxy> getFallbacks() {
        List<FallbackProxy> allFallbacks = new ArrayList<FallbackProxy>(
                fallbacks);
        // Remove all flashlights
        Iterator<FallbackProxy> it = allFallbacks.iterator();
        while (it.hasNext()) {
            FallbackProxy proxy = it.next();
            if (PtType.FLASHLIGHT == proxy.getPtType()) {
                it.remove();
            }
        }
        allFallbacks.add(flashlightProxy("default.peer.getiantem.org", -1));
        allFallbacks.add(flashlightProxy("default.fallback.getiantem.org", 1));
        return allFallbacks;
    }
    ...
    public static Properties flashlightProps(String host) {
        ...
        props.setProperty(Flashlight.ROOT_CA_KEY,
                FASTLY_DIGICERT_HIGH_ASSURANCE_CA_3);
        return props;
    }
