...
HRESULT windows_firewall_initialize_ascom(OUT INetFwPolicy2** policy, IN BOOL as_admin)
{
    HRESULT hr = S_OK;
...
void windows_firewall_cleanup_ascom(IN INetFwPolicy2 *policy)
{
    if (policy != NULL) {
...
HRESULT windows_firewall_is_on_ascom(IN INetFwPolicy2 *policy, OUT BOOL *fw_on)
{
    HRESULT hr = S_OK;
...
//  Turn Firewall ON
HRESULT windows_firewall_turn_on_ascom(IN INetFwPolicy2 *policy)
{
    HRESULT hr = S_OK;
    BOOL fw_on;
    ...
    RETURN_IF_FAILED(windows_firewall_is_on_ascom(policy, &fw_on));
...
//  Turn Firewall OFF
HRESULT windows_firewall_turn_off_ascom(IN INetFwPolicy2 *policy)
{
    HRESULT hr = S_OK;
    BOOL fw_on;
    ...
    // Check the current firewall status first
    hr = windows_firewall_is_on_ascom(policy, &fw_on);
    RETURN_IF_FAILED(hr);
