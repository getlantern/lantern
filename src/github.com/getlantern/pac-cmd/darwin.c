...
int togglePac(bool turnOn, const char* pacUrl)
{
    ...
    if(prefsRef==NULL) {
        NSLog(@"pac-cmd failed to obtain Preferences Ref");
        ret = NO_PERMISSION;
        goto freePrefsRef;
    }
    ...
    if (!success) {
        NSLog(@"pac-cmd failed to obtain PreferencesLock");
        ret = NO_PERMISSION;
        goto freePrefsRef;
    }
    ...
    if(networkSetRef == NULL) {
        NSLog(@"pac-cmd failed to get available network services");
        ret = SYSCALL_FAILED;
    ...
    for (long i = 0; i < CFArrayGetCount(networkServicesArrayRef); i++) {
        ...
        if(proxyProtocolRef == NULL) {
            NSLog(@"pac-cmd Couldn't acquire copy of proxyProtocol");
            ret = SYSCALL_FAILED;
        ...
        if(!success) {
            NSLog(@"pac-cmd failed to set protocol configuration");
            ret = SYSCALL_FAILED;
            goto freeProxyProtocolRef;
        } else {
            NSLog(@"pac-cmd successfully set protocol configuration");
        }
    ...
    if(!success) {
        NSLog(@"pac-cmd ailed to commit changes");
        ret = SYSCALL_FAILED;
    ...
    if(!success) {
        NSLog(@"pac-cmd failed to apply changes");
        ret = SYSCALL_FAILED;
        goto freeNetworkServicesArrayRef;
    }
