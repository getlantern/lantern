...
Android stealth manifest minimization is also available to local Gradle
experiments through `-PSTEALTH_MODE=vpn` or `-PSTEALTH_MODE=novpn`. The Gradle
task uses `-PstealthPython`, then `PYTHON`, then `python3` to generate the
filtered manifest. `-PstealthNoVpn=true` is kept as a compatibility switch for
older automation: when `STEALTH_MODE` is unset it selects `novpn`, and when
combined with `STEALTH_MODE=vpn` Gradle fails fast because the inputs conflict.
Prefer `STEALTH_MODE=novpn` for new scripts.
