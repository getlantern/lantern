(function() {
  ...
  function d3_geo_compose(a, b) {
    function compose(Î», Ï†) {
      var coordinates = a(Î», Ï†);
  ...
  function d3_geo_graticuleX(y0, y1) {
    var y = d3.range(y0, y1 - Îµ, d3_geo_graticulePrecision).concat(y1);
    return function(x) {
  ...
  function d3_geo_graticuleY(x0, x1) {
    var x = d3.range(x0, x1 - Îµ, d3_geo_graticulePrecision).concat(x1);
    return function(y) {
  ...
  function d3_geo_projectionMutator(projectAt) {
    function projection(coordinates) {
      ...
      return [ coordinates[0] * k + Î´x, Î´y - coordinates[1] * k ];
    }
    function invert(coordinates) {
      coordinates = projectRotate.invert((coordinates[0] - Î´x) / k, (Î´y - coordinates[1]) / k);
      return [ coordinates[0] * d3_degrees, coordinates[1] * d3_degrees ];
    }
    function resample(context) {
      function point(Î», Ï†) {
        var p = projectPoint(Î», Ï†);
        context.point(p[0], p[1]);
      }
      function moveTo(Î», Ï†) {
        var p = projectPoint(Î»00 = Î»0 = Î», Ï†00 = Ï†0 = Ï†);
        context.moveTo(x0 = p[0], y0 = p[1]);
      }
      function lineTo(Î», Ï†) {
        var p = projectPoint(Î», Ï†);
        resampleLineTo(x0, y0, Î»0, Ï†0, x0 = p[0], y0 = p[1], Î»0 = Î», Ï†0 = Ï†, maxDepth);
        context.lineTo(x0, y0);
      }
      function resampleLineTo(x0, y0, Î»0, Ï†0, x1, y1, Î»1, Ï†1, depth) {
        var dx = x1 - x0, dy = y1 - y0, distance2 = dx * dx + dy * dy;
        if (distance2 > 4 * Î´2 && depth--) {
          var sinÏ†0 = Math.sin(Ï†0), cosÏ†0 = Math.cos(Ï†0), sinÏ†1 = Math.sin(Ï†1), cosÏ†1 = Math.cos(Ï†1), cosÎ© = sinÏ†0 * sinÏ†1 + cosÏ†0 * cosÏ†1 * Math.cos(Î»1 - Î»0), k = 1 / (Math.SQRT2 * Math.sqrt(1 + cosÎ©)), x = k * (cosÏ†0 * Math.cos(Î»0) + cosÏ†1 * Math.cos(Î»1)), y = k * (cosÏ†0 * Math.sin(Î»0) + cosÏ†1 * Math.sin(Î»1)), z = k * (sinÏ†0 + sinÏ†1), Ï†2 = Math.asin(Math.max(-1, Math.min(1, z))), Î»2 = Math.abs(x) < Îµ || Math.abs(y) < Îµ ? (Î»0 + Î»1) / 2 : Math.atan2(y, x), p = projectPoint(Î»2, Ï†2), x2 = p[0], y2 = p[1], dx2 = x0 - x2, dy2 = y0 - y2, dz = dx * dy2 - dy * dx2, Î´distance2 = Math.abs(dx2 * dx2 + dy2 * dy2 - distance2);
          if (Î´distance2 < Îµ) {
            resampleLineTo(x0, y0, Î»2, Ï†2, x1, y1, Î»1, Ï†1, depth);
          } else if (Î´distance2 > distance2 - Îµ) {
            resampleLineTo(x0, y0, Î»0, Ï†0, x1, y1, Î»2, Ï†2, depth);
          } else if (dz * dz / distance2 > Î´2) {
            resampleLineTo(x0, y0, Î»0, Ï†0, x2, y2, Î»2, Ï†2, depth);
            context.lineTo(x2, y2);
            resampleLineTo(x2, y2, Î»2, Ï†2, x1, y1, Î»1, Ï†1, depth);
          }
        }
      }
      function closePath() {
        var p = projectPoint(Î»00, Ï†00);
        resampleLineTo(x0, y0, Î»0, Ï†0, p[0], p[1], Î»00, Ï†00, maxDepth);
        context.closePath();
      }
      var Î»00, Ï†00, Î»0, Ï†0, x0, y0, maxDepth = Î´2 > 0 && 16;
      return {
        point: point,
        moveTo: moveTo,
        lineTo: lineTo,
    ...
    function projectPoint(Î», Ï†) {
      var point = project(Î», Ï†);
    ...
    var project, rotate, projectRotate, k = 150, x = 480, y = 250, Î» = 0, Ï† = 0, Î´Î» = 0, Î´Ï† = 0, Î´Î³ = 0, Î´x = x, Î´y = y, Î´2 = .5, clip = d3_geo_projectionCutAntemeridian(rotatePoint), clipAngle = null;
  ...
  function d3_geo_projectionIntersectAntemeridian(Î»0, Ï†0, Î»1, Ï†1) {
  ...
  function d3_geo_projectionCutAntemeridian(rotatePoint) {
    var clip = {
      ...
      line: function(coordinates, context) {
        if (!(n = coordinates.length)) return;
        var point = rotatePoint(coordinates[0]), Î»0 = point[0], Ï†0 = point[1], Î»1, Ï†1, sÎ»0 = Î»0 > 0 ? Ï€ : -Ï€, sÎ»1, i = 0, n;
        context.moveTo(Î»0, Ï†0);
        while (++i < n) {
          point = rotatePoint(coordinates[i]);
          Î»1 = point[0];
          Ï†1 = point[1];
          sÎ»1 = Î»1 > 0 ? Ï€ : -Ï€;
          if (sÎ»0 !== sÎ»1 && Math.abs(Î»1 - Î»0) >= Ï€) {
            Ï†0 = d3_geo_projectionIntersectAntemeridian(Î»0, Ï†0, Î»1, Ï†1);
            if (Math.abs(Î»0 - sÎ»0) > Îµ) context.lineTo(sÎ»0, Ï†0);
            if (Math.abs(Î»1 - sÎ»1) > Îµ) context.moveTo(sÎ»1, Ï†0), context.lineTo(Î»0 = Î»1, Ï†0 = Ï†1); else context.moveTo(Î»0 = Î»1, Ï†0 = Ï†1);
          } else {
            context.lineTo(Î»0 = Î»1, Ï†0 = Ï†1);
          }
          sÎ»0 = sÎ»1;
        }
      },
  ...
  function d3_geo_antemeridianAngle(point) {
    return -(point[0] < 0 ? point[1] - Ï€ / 2 : Ï€ / 2 - point[1]);
  }
  function d3_geo_antemeridianInterpolate(from, to, context) {
    from = from.point;
    to = to.point;
    if (Math.abs(from[0] - to[0]) > Îµ) {
      var s = from[0] < to[0] ? Ï€ : -Ï€, Ï† = s / 2;
      context.lineTo(-s, Ï†);
      context.lineTo(0, Ï†);
      context.lineTo(s, Ï†);
    } else {
  ...
  function d3_geo_forwardRotationÎ»(Î´Î») {
    return function(Î», Ï†) {
      return [ (Î» += Î´Î») > Ï€ ? Î» - 2 * Ï€ : Î» < -Ï€ ? Î» + 2 * Ï€ : Î», Ï† ];
    };
  }
  ...
  var Ï€ = Math.PI, Îµ = 1e-6, d3_radians = Ï€ / 180, d3_degrees = 180 / Ï€, d3_zero = function() {
    return 0;
  };
  ...
  d3.geo.path = function() {
    ...
    function lineCentroid(coordinates) {
      ...
      var n, point = projection(coordinates[0]), x0 = point[0], y0 = point[1], x1, y1, dx, dy, x = 0, y = 0, z = 0, i = 0, Î´;
      while (++i < n) {
        x1 = (point = projection(coordinates[i]))[0];
        y1 = point[1];
        dx = x1 - x0;
        dy = y1 - y0;
        z += Î´ = Math.sqrt(dx * dx + dy * dy);
        x += Î´ * (x0 + x1) / 2;
        y += Î´ * (y0 + y1) / 2;
        x0 = x1;
        y0 = y1;
