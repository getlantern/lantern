...
type User interface {
	...
	DeleteAccount(email, password string, oAuthProvider bool) ([]byte, error)
...
func (lc *LanternCore) DeleteAccount(email, password string, oAuthProvider bool) ([]byte, error) {
	slog.Debug("Deleting account")
	return lc.apiClient.DeleteAccount(context.Background(), email, password, oAuthProvider)
}
